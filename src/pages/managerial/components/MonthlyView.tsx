    ...
    <div className="space-y-4 sm:space-y-6 animate-in fade-in duration-500 w-full">
      ...
      <div className="bg-black/40 border border-white/5 p-4 sm:p-8 rounded-2xl backdrop-blur-sm">
        <div className="flex items-center justify-between max-w-lg mx-auto">
          <button 
            ...
            className="text-white/30 hover:text-cyan-400 transition-all text-xl sm:text-2xl p-2 sm:p-3"
          >
            ◀
          </button>
        ...
        <div className="flex items-center justify-center gap-4 sm:gap-6 mt-4 sm:mt-8">
          <button 
            ...
            className="text-white/30 hover:text-cyan-400 transition-all text-lg sm:text-xl p-2 sm:p-3"
          >
            ◀
          </button>
          
          <span className="text-xl sm:text-2xl font-black tracking-tight text-white px-6 sm:px-10 min-w-[100px] sm:min-w-[140px] text-center">
            {selectedYear}
          </span>
      ...
      <div className="flex justify-start">
        <button 
          ...
          className="w-full sm:w-auto flex items-center justify-center gap-3 px-6 sm:px-8 py-3 sm:py-4 bg-white/5 border border-white/10 rounded-xl text-white hover:bg-white/10 hover:border-cyan-500/50 transition-all active:scale-95 shadow-xl group"
        >
      ...
      <div className="bg-black/40 border border-white/5 rounded-2xl overflow-hidden">
        <button
          ...
          className="w-full flex items-center justify-between p-4 sm:p-6 text-white hover:bg-white/[0.02] transition-colors"
        >
          <div className="flex items-center gap-3 sm:gap-4">
          ...
          <div className="p-4 sm:p-6 border-t border-white/5 space-y-4 sm:space-y-6 animate-in slide-in-from-top-4">
            <form onSubmit={onAddHoliday} className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              <div className="space-y-4 sm:space-y-5">
                <div className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 bg-white/5 rounded-xl border border-white/5">
                  <input
                    ...
                    className="w-4 h-4 sm:w-5 sm:h-5 rounded border-white/10 bg-black text-cyan-500 focus:ring-cyan-500"
                  />
                  <label htmlFor="multiDayMonthly" className="text-xs sm:text-sm font-bold text-white/60 uppercase tracking-wider cursor-pointer">
                ...
                <div className="grid grid-cols-2 gap-3 sm:gap-4">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className="w-full bg-black/50 border border-white/10 rounded-xl p-3 sm:p-4 text-sm sm:text-base text-white focus:border-cyan-500 outline-none"
                      required
                    />
                  </div>
                  {isMultiDay && (
              ...
              <div className="space-y-4 sm:space-y-5">
                ...
                <button
                  type="submit"
                  className="w-full py-3 sm:py-4 bg-cyan-500/20 border border-cyan-500/50 rounded-xl text-cyan-400 text-xs sm:text-sm font-black tracking-[0.1em] sm:tracking-[0.2em] uppercase hover:bg-cyan-500/30 transition-all shadow-lg"
                >
                  SIMPAN KE DATABASE
                </button>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 pt-4 sm:pt-6 border-t border-white/5">
              {holidays.length > 0 ? (
                holidays.map((h) => (
                  <div key={h.id} className="flex justify-between items-center p-3 sm:p-4 bg-white/[0.03] border border-white/5 rounded-xl group hover:border-cyan-500/30 transition-all">
                    ...
                    <button
                      ...
                      className="p-2 sm:p-3 text-lg sm:text-xl text-red-400 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity hover:bg-red-500/10 rounded-lg"
                    >
                      🗑️
                    </button>
                  </div>
                ...
                <div className="col-span-full py-4 sm:py-6 text-center text-white/20 text-xs sm:text-sm font-bold tracking-widest uppercase">
