...
// Updated: "ontime" is replaced by "healthy"
...
export function AnalisaKehadiran() {
  ...
  const uniqueTodayRecords = useMemo(() => {
    ...
    for (const record of sorted) {
      if (!unique.has(record.employeeId)) unique.set(record.employeeId, record);
    }
  ...
  const monthlyStats = useMemo((): MonthlyStats[] => {
    ...
    return employees.map(emp => {
      ...
      sortedEmpRecords.forEach(r => {
        ...
        if (!recordDates.has(dateKey)) recordDates.set(dateKey, r);
      });
      
      const dailyStats: DailyStats[] = [];
      let healthy = 0, late = 0, absent = 0, sick = 0, symptom = 0, bizDays = 0, attendedBizDays = 0;
      ...
      for (let i = 1; i <= lastDayOfMonth.getDate(); i++) {
        ...
        const isHol = expandedHolidayDates.has(dKey);
        const isBizDay = !isWknd && !isHol; 
        ...
        if (record) {
            ...
            // PRIORITY 1: Absent (Submitted way too late)
            if (recordHour >= 17) {
                status = 'absent';
                if (isBizDay) absent++; 
            } 
            // PRIORITY 2: Sick
            else if (record.healthCondition === 'sick-checked-medical') {
                status = 'sick';
                sick++;
                if (isBizDay) attendedBizDays++; 
            } 
            // PRIORITY 3: Late
            else if (getAttendanceStatus(record.timestamp) === 'late') {
                status = 'late';
                late++;
                if (isBizDay) attendedBizDays++;
            } 
            // PRIORITY 4: Has Symptoms (but was on time)
            else if (record.healthCondition === 'has-symptoms-not-checked') {
                status = 'symptom';
                symptom++;
                if (isBizDay) attendedBizDays++;
            } 
            // PRIORITY 5: Healthy (On time, no symptoms)
            else {
                status = 'healthy';
                healthy++;
                if (isBizDay) attendedBizDays++;
      ...
      return {
        ...
        attendanceRate: bizDays > 0 ? Math.round((attendedBizDays / bizDays) * 100) : 0,
        dailyStats
      };
    });
  ...
  const handleAddHoliday = async (e: React.FormEvent) => {
    ...
    const newHoliday: Holiday = {
      id: Date.now().toString(), date: holidayDate, name: holidayName, isCustom: true, isMultiDay,
      ...(isMultiDay && { endDate: holidayEndDate })
    };
    try {
      await addHoliday(newHoliday);
      setHolidayName(''); setHolidayDate(''); setHolidayEndDate(''); setShowHolidayForm(false);
    } catch (err) { console.error("Gagal tambah libur:", err); }
  };
  ...
  const handleExportExcel = useCallback(() => {
    ...
    let colDefinitions = `<col width="250">`; 
    for (let i = 1; i <= daysInMonth; i++) colDefinitions += `<col width="35">`; 
    colDefinitions += `<col width="80">`;
    ...
    monthlyStats.forEach((stat) => {
      ...
      for (let i = 1; i <= daysInMonth; i++) {
        ...
        let cellContent = ''; let bgColor = '#ffffff'; let fontColor = '#000000';

        if (st === 'weekend' || st === 'holiday') {
            bgColor = '#e7e6e6'; 
        } else {
            // EXCEL EXPORT - Clean 1-Color Mapping
            if (st === 'healthy') {
                cellContent = 'V'; bgColor = '#3cb371'; fontColor = '#000000'; // Green
                totalDays++; dailyTotals[i-1]++;
            } else if (st === 'late') {
                cellContent = 'L'; bgColor = '#a855f7'; fontColor = '#ffffff'; // Purple
                totalDays++; dailyTotals[i-1]++;
            } else if (st === 'symptom') {
                cellContent = 'H'; bgColor = '#f97316'; fontColor = '#ffffff'; // Orange
                totalDays++; dailyTotals[i-1]++;
            } else if (st === 'sick') {
                cellContent = 'S'; bgColor = '#4169e1'; fontColor = '#ffffff'; // Blue
                totalDays++; dailyTotals[i-1]++;
            } else if (st === 'absent') {
                cellContent = 'A'; bgColor = '#ff0000'; fontColor = '#ffffff'; // Red
            }
        }
      ...
      rowHtml += `<td style="border: 1px solid #ffffff; background-color: #f2f2f2; text-align:center; font-weight:bold; font-family: Calibri, sans-serif;">${totalDays}</td></tr>`;
      employeeRows += rowHtml; grandTotal += totalDays;
    });
    ...
    <head><meta charset="utf-8"><style>table { border-collapse: collapse; } td { white-space: nowrap; vertical-align: middle; }</style></head>
    <body>
      <table>
        ${colDefinitions} <tr height="40"><td colspan="5" style="font-family: Calibri, sans-serif; font-size: 28pt; font-weight: bold; color: #4472c4; vertical-align: middle;">${currentMonthEn}</td><td colspan="${daysInMonth - 3}"></td></tr>
        ...
        <tr height="25">
          ...
          <td style="background-color:#3cb371; text-align:center; font-weight:bold; border: 1px solid #fff;">V</td><td style="font-family: Calibri, sans-serif; padding-left:5px;">Healthy</td>
          <td style="background-color:#a855f7; color:white; text-align:center; font-weight:bold; border: 1px solid #fff;">L</td><td style="font-family: Calibri, sans-serif; padding-left:5px;">Late</td>
          <td style="background-color:#f97316; color:white; text-align:center; font-weight:bold; border: 1px solid #fff;">H</td><td style="font-family: Calibri, sans-serif; padding-left:5px;">Symptoms</td>
          <td style="background-color:#4169e1; color:white; text-align:center; font-weight:bold; border: 1px solid #fff;">S</td><td style="font-family: Calibri, sans-serif; padding-left:5px;">Sick</td>
          <td style="background-color:#ff0000; color:white; text-align:center; font-weight:bold; border: 1px solid #fff;">A</td><td style="font-family: Calibri, sans-serif; padding-left:5px;" colspan="${daysInMonth - 9}">Absent</td>
        </tr>
        <tr height="15"><td colspan="${daysInMonth + 2}" style="border-bottom: 2px solid #000;"></td></tr>
        <tr height="30"><td colspan="${daysInMonth}" style="font-family: Calibri, sans-serif; font-size: 16pt; font-weight: bold; vertical-align: bottom;">Dates of Absence</td><td colspan="2" style="font-family: Calibri, sans-serif; font-size: 16pt; font-weight: bold; text-align: center; vertical-align: bottom;">${selectedYear}</td></tr>
        ${dowRow}${dateRow}${employeeRows}
    ...
    a.href = url; a.download = `Data_Matrix_${currentMonthEn}_${selectedYear}.xls`;
    ...
    <div className="min-h-screen bg-black text-white p-4 space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 border-b border-white/5 pb-4 sm:pb-6">
        <div className="w-full sm:w-auto">
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-black tracking-tighter italic uppercase text-white break-words">Analisa Kehadiran</h1>
          <p className="text-xs sm:text-sm font-bold tracking-[0.2em] sm:tracking-[0.4em] text-cyan-500 uppercase mt-2">Data Matrix v2.0</p>
        </div>
        <div className="w-full sm:w-auto bg-white/5 p-4 sm:p-5 rounded-2xl border border-white/10 flex flex-row sm:flex-col justify-between items-center sm:items-end">
          ...
          <p className="text-2xl sm:text-3xl font-black text-white mt-0 sm:mt-1">
            {monthlyStats.length > 0 ? Math.round(monthlyStats.reduce((a, b) => a + b.attendanceRate, 0) / monthlyStats.length) : 0}%
          </p>
        </div>
      </div>
      ...
      <div className="flex bg-white/5 p-1 sm:p-2 rounded-xl sm:rounded-2xl border border-white/5">
        {(['harian', 'bulanan'] as TabType[]).map(tab => (
          <button key={tab} onClick={() => handleTabChange(tab)} className={`flex-1 py-3 sm:py-4 text-xs sm:text-sm font-black tracking-widest rounded-lg sm:rounded-xl transition-all ${activeTab === tab ? 'bg-white text-black' : 'text-white/40 hover:text-white'}`}>
            {tab.toUpperCase()}
          </button>
        ))}
      </div>

      <ConnectionStatusBar isConnected={isConnected} lastSyncTime={lastSyncTime} isRefreshing={isRefreshing} onRefresh={handleRefresh} />

      {isPending ? (
        <div className="py-20 text-center animate-pulse"><span className="text-xs font-bold tracking-[0.5em] text-cyan-500">PROCESSING DATA...</span></div>
      ) : activeTab === 'harian' ? (
        <DailyView dailyDate={dailyDate} onDailyDateChange={setDailyDate} todayRecords={uniqueTodayRecords} employeesWhoDidNotSubmit={employeesWhoDidNotSubmitToday} employees={employees} expandedCard={expandedCard} onToggleCard={setExpandedCard} getAttendanceStatus={getAttendanceStatus} getHealthColor={getHealthColor} />
      ) : (
        <MonthlyView selectedMonth={selectedMonth} selectedYear={selectedYear} onMonthChange={setSelectedMonth} onYearChange={setSelectedYear} monthlyStats={monthlyStats} employees={employees} holidays={holidays} showHolidayForm={showHolidayForm} isMultiDay={isMultiDay} holidayDate={holidayDate} holidayEndDate={holidayEndDate} holidayName={holidayName} onToggleHolidayForm={() => setShowHolidayForm(!showHolidayForm)} onMultiDayChange={setIsMultiDay} onHolidayDateChange={setHolidayDate} onHolidayEndDateChange={setHolidayEndDate} onHolidayNameChange={setHolidayName} onAddHoliday={handleAddHoliday} onDeleteHoliday={deleteHoliday} onExportExcel={handleExportExcel} />
      )}
    </div>
  );
}
